...
export function rateCheck(ip, amount = 1) {
  ...
  const used = arr.reduce((sum, e) => sum + e.amount, 0);
...
export async function sendLike(targetUid, region = "sg", count = 1) {
  ...
  try {
    ...
    throw new Error("ffapis require failed: " + (err.message || String(err)));
  }
  ...
  if (!LikeAPI) {
    throw new Error(
      "LikeAPI missing. Exports: " + Object.keys(ff).join(",")
    );
  }

  const api = new LikeAPI();
  const ob = DEFAULT_OB_VERSION || "OB54";
  const reqCount = Math.max(1, Math.min(Number(count) || 1, 40));

  const result = await api.sendLikes(
    String(targetUid),
    region,
    reqCount,
    ob
